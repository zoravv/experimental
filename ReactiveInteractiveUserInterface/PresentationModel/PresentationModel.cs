...
namespace TP.ConcurrentProgramming.Presentation.Model
{
    ...
    internal class ModelImplementation : ModelAbstractApi
    {
        ...
        public override void SetScale(double availableWidth, double availableHeight)
        {
            if (availableWidth <= 0)
            ...
            double widthScale = availableWidth / UnderneathLayerAPI.GetDimensions.TableWidth;
            double heightScale = availableHeight / UnderneathLayerAPI.GetDimensions.TableHeight;
            scale = Math.Clamp(Math.Min(widthScale, heightScale), 0.50, 1.80);
        }

        public override double Scale => scale;
        ...
        {
            ModelBall newBall = new ModelBall(position.x * scale, position.y * scale, scale, ball)
            {
